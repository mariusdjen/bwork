...
export async function POST(request: NextRequest) {
	try {
		...
		if (
			sandbox.constructor?.name === "VercelProvider" ||
			sandbox.getSandboxInfo?.()?.provider === "vercel"
		) {
			console.log(
				"[apply-ai-code] Detected Vercel sandbox, checking Vite status..."
			);
			try {
				...
				if (!checkResult || !checkResult.stdout) {
					...
					await sandbox.runCommand(
						'sh -c "cd /vercel/sandbox && nohup npm run dev > /tmp/vite.log 2>&1 &"'
					);
					// Wait for Vite to start
					await new Promise((resolve) => setTimeout(resolve, 5000));
					console.log(
						"[apply-ai-code] Vite started, proceeding with code application"
					);
				} else {
				...
				console.log(
					"[apply-ai-code] Could not check Vite status, proceeding anyway:",
					e
				);
			}
		}
		...
		if (morphEnabled) {
			...
			if (morphEdits.length > 0) {
				console.log(
					"[apply-ai-code] Parsed Morph edits:",
					morphEdits.map((e) => e.targetFile)
				);
			} else {
		...
		if (morphEnabled && morphEdits.length > 0) {
			if (!global.activeSandbox) {
				console.warn(
					"[apply-ai-code] Morph edits found but no active sandbox; skipping Morph application"
				);
			} else {
				console.log(
					`[apply-ai-code] Applying ${morphEdits.length} fast edits via Morph...`
				);
				for (const edit of morphEdits) {
					try {
						...
						if (result.success && result.normalizedPath) {
							...
							console.log(
								"[apply-ai-code] Morph applied to",
						...
						results.errors.push(
							`Morph apply exception for ${edit.targetFile}: ${
								(e as Error).message
							}`
						);
					}
		...
		if (morphEnabled && morphEdits.length === 0) {
			console.warn(
				"[apply-ai-code] Morph enabled but no <edit> blocks found; falling back to full-file flow"
			);
		}
