...
function AISandboxPage() {
	...
	const [briefPromptText, setBriefPromptText] = useState<string | null>(null);
	const [toolSaved, setToolSaved] = useState(false);
	const lastSavedRef = useRef<string | null>(null);
	const generationJobRef = useRef<string | null>(null);
	const saveTimerRef = useRef<NodeJS.Timeout | null>(null);
	const prevBusyRef = useRef<boolean>(false);
	const smokeTestCacheRef = useRef<Set<string>>(new Set());

	const cleanPromptText = (text?: string | null, max?: number) => {
		if (!text) return "";
		let t = text.replace(/Brief métier:\s*/i, "").trim();
	...
	const [hasSentBrief, setHasSentBrief] = useState(false);
	...
	useEffect(() => {
		...
		const initializePage = async () => {
			...
			setLoading(true);
			try {
				// Toujours créer un nouveau sandbox (pas de restauration)
				console.log("[home] Création d'un nouveau sandbox (pas de restore)");
				sandboxCreated = true;
	...
	// Auto-trigger generation quand flag depuis home (URL) ou brief prompt
	useEffect(() => {
		const brief = sessionStorage.getItem("briefPrompt");
		if (brief && !hasSentBrief) {
			setHasInitialSubmission(true);
			setShowHomeScreen(false);
			setHomeScreenFading(false);
			setActiveTab("preview");
			setHasSentBrief(true);
			setBriefMode(true);
	...
	useEffect(() => {
		const sandboxUrl =
			sandboxData?.url ||
			// fallback possibles selon payload sandbox
			(sandboxData as any)?.previewUrl ||
			(sandboxData as any)?.sandboxUrl;
		const busy =
			generationProgress.isGenerating ||
			Boolean(codeApplicationState.stage) ||
			loading ||
			isStartingNewGeneration ||
			isPreparingDesign ||
			isCapturingScreenshot;

		// Toast fin de génération
		if (prevBusyRef.current && !busy && sandboxUrl) {
			toast.success("Outil généré", {
				description: "La preview est prête.",
				action: {
					label: "Ouvrir",
					onClick: () => window.open(sandboxUrl, "_blank", "noreferrer"),
				},
			});
		}
		...
		// Lance une sauvegarde différée seulement si URL valide et génération finie
		if (sandboxUrl && /^https?:\/\//i.test(sandboxUrl) && !busy) {
			const key = sandboxData?.sandboxId || sandboxUrl;
			saveTimerRef.current = setTimeout(async () => {
				if (!key) return;
				if (
					lastSavedRef.current === key ||
					sessionStorage.getItem(`tool_saved_${key}`) === "true"
				) {
					return;
				}
				const title =
					cleanPromptText(briefPromptText, 60) ||
					`Outil généré - ${new Date().toLocaleString("fr-FR")}`;
				const description =
					cleanPromptText(briefPromptText, 240) || "Outil généré via B-WORK";
				try {
					// On teste le build mais on n'empêche plus la sauvegarde en cas d'échec
					void runSmokeTest(key);
					...
					if (!res.ok || !data?.ok) {
						console.warn("save tool error", data?.error || res.statusText);
						return;
					}
					setToolSaved(true);
					lastSavedRef.current = key;
					sessionStorage.setItem(`tool_saved_${key}`, "true");
					await registerGenerationStatus("done", {
						sandboxUrl: sandboxUrl,
					});
				} catch (err) {
					console.warn("save tool error", err);
				}
			}, 1800); // petit délai pour éviter les URLs intermédiaires
		}
	...
	useEffect(() => {
		// reset flag when sandbox changes
		const key = sandboxData?.sandboxId || sandboxData?.url || null;
		if (!key) {
			sessionStorage.removeItem("tool_saved_last");
			lastSavedRef.current = null;
			setToolSaved(false);
			return;
		} else if (lastSavedRef.current !== key) {
			setToolSaved(false);
		}
	...
	const registerGenerationStatus = async (
		status: "running" | "done",
		payload?: { sandboxUrl?: string; title?: string }
	) => {
		...
		try {
			const res = await fetch("/api/generations", {
				...
				body: JSON.stringify({
					...
					status,
					sandbox_url: payload?.sandboxUrl,
					title:
						payload?.title ||
						cleanPromptText(briefPromptText, 60) ||
						"Outil généré (background)",
				}),
			});
			const data = await res.json();
			if (!res.ok || !data?.ok) {
				console.warn(
					"register generation status error",
					data?.error || res.statusText
				);
				return;
			}
			generationJobRef.current = data.id || generationJobRef.current;
		} catch (err) {
			console.warn("register generation status error", err);
		}
	};

	const runSmokeTest = async (key: string) => {
		if (!key) return true;
		if (smokeTestCacheRef.current.has(key)) return true;

		try {
			const res = await fetch("/api/sandbox/smoke-test", {
				method: "POST",
				headers: { "Content-Type": "application/json" },
				body: JSON.stringify({
			...
			if (!res.ok || !data?.ok) {
				const msg =
					data?.stderr?.slice(0, 600) ||
					data?.error ||
					"Le test automatique a échoué.";
				console.warn("smoke test failed:", msg);
				return false;
			}
			smokeTestCacheRef.current.add(key);
			return true;
		} catch (error) {
			console.warn("smoke test error", error);
			return false;
		}
	};
...
Tip: I automatically detect and install npm packages from your code imports (like react-router-dom, axios, etc.)`,
	...
	const renderMainContent = () => {
		if (briefMode) {
			const briefSnippet =
				briefPromptText && briefPromptText.length > 700
					? `${briefPromptText.slice(0, 700)}...`
					: briefPromptText;
			const showIframe =
				sandboxData?.url &&
				!generationProgress.isGenerating &&
					...
					{showIframe ? (
						...
						<div className="absolute inset-0 flex items-center justify-center">
							<div className="text-center text-gray-600">
								<div className="w-12 h-12 border-4 border-heat-100 border-t-transparent rounded-full animate-spin mx-auto mb-3" />
								<p className="text-sm text-black-alpha-72">
									Nous préparons ton espace de prévisualisation...
								</p>
							</div>
						</div>
					)}

					<div className="absolute inset-0 bg-white/85 backdrop-blur-md flex flex-col items-center justify-center gap-4 z-10 px-6 text-center">
						{isBusy ? (
							<>
								...
								<div className="flex flex-wrap justify-center gap-2">
									<button
										onClick={() => {
											...
											router.push("/dashboard/user/tools-list");
										}}
										className="inline-flex items-center gap-2 px-3 py-2 rounded-md border border-border-faint bg-white text-xs font-medium text-accent-black hover:bg-border-faint transition-colors"
									>
	...
	const sendChatMessage = async (messageOverride?: string) => {
		...
		try {
			...
			if (generatedCode) {
				...
				if (sandboxPromise) {
					setBriefStatus("Préparation de l'aperçu...");
				...
				if (activeSandboxData && generatedCode) {
					...
					if (sandboxCreating) {
						...
						setBriefStatus("Sandbox prêt, démarrage du serveur de preview...");
						await new Promise((resolve) => setTimeout(resolve, 2000));
					}
					...
					setBriefStatus("Outil prêt ! Prévisualisation disponible.");
				}
			}
...
Focus on the key sections and content, making it clean and modern.`;
	...
	return (
		<HeaderProvider>
			<div className="font-sans text-foreground min-h-[calc(100vh-120px)] bg-gradient-to-br from-slate-50 via-white to-slate-100 flex flex-col gap-4 p-4 md:p-6 rounded-2xl border border-slate-200 shadow-sm">
				<div className="rounded-xl border bg-white px-4 md:px-6 py-3 flex items-center justify-between shadow-sm">
					<div className="text-lg font-semibold tracking-tight text-accent-black px-4">
						B-WORK
					</div>
					<div className="flex items-center gap-2">
						...
						<button
							...
							className="px-3 py-2 rounded-lg transition-colors bg-gray-50 border border-gray-200 text-gray-700 hover:bg-gray-100 disabled:opacity-50 disabled:cursor-not-allowed"
				...
				<div className="flex-1 flex overflow-hidden rounded-xl border bg-white shadow-sm">
					...
					<div className="flex-1 flex flex-col overflow-hidden">
						<div className="px-3 pt-4 pb-4 bg-white border-b border-gray-200 flex justify-between items-center">
							<div className="flex items-center gap-2">
								...
								{briefMode ? (
									<div className="inline-flex items-center gap-2 px-3 py-1.5 bg-gray-100 border border-gray-200 rounded-md text-xs font-medium text-gray-800">
										<div className="w-1.5 h-1.5 bg-heat-100 rounded-full animate-pulse" />
										Génération guidée B-WORK
									</div>
								) : (
							...
							<div className="flex gap-2 items-center">
								<button
									onClick={() => {
										sessionStorage.removeItem("briefPrompt");
										sessionStorage.removeItem("autoStart");
										router.push("/generation/brief");
									}}
									className="inline-flex items-center gap-2 px-3 py-2 rounded-md border border-border-faint bg-white text-xs font-medium text-accent-black hover:bg-border-faint transition-colors"
