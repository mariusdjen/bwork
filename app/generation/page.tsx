...
function AISandboxPage() {
  ...
  useEffect(() => {
	...
	useEffect(() => {
		...
		if (
			...
			saveTimerRef.current = setTimeout(async () => {
				...
				const title =
					briefPromptText?.slice(0, 60) ||
					`Outil généré - ${new Date().toLocaleString("fr-FR")}`;
				const description =
					briefPromptText?.slice(0, 240) || "Outil généré via B-WORK";
				try {
	...
	}, [sandboxData?.url, sandboxData?.sandboxId, generationProgress.isGenerating, briefPromptText]);
  ...
  const updateStatus = (text: string, active: boolean) => {
	...
	const registerGenerationStatus = async (
		...
		try {
			...
			const data = await res.json();
			if (!res.ok || !data?.ok) {
				console.warn("register generation status error", data?.error || res.statusText);
				return;
			}
